 ...
 */
function saveGameResult(gameName: string, steps: number, score: number, time: number, completed: boolean): void {
  // Use the game name directly as filename (CLI names are already clean)
  const resultsPath = resolve(process.cwd(), `${gameName}.json`);
  let results: Record<string, Array<{ steps: number; score: number; time: number; completed: boolean; timestamp: string }>> = {};
  ...
  if (!provider) {
    console.warn('Warning: No provider set, cannot save results');
    return;
  }
 ...
 */
async function playGame(gameDefinition: GameDefinition, gameKey: string): Promise<{
  success: boolean;
  result?: any;
  finalScore?: number;
  error?: string;
  ...
  try {
    ...
    if (isCompleted) {
      ...
      console.log(chalk.yellow('⚠️ MISSION INCOMPLETE!\n'));
    }

    const duration = ((Date.now() - startTime) / 1000).toFixed(1);
    const moveCount = playthrough.finalSteps.length;
    console.log(chalk.green(`✅ PLAYTHROUGH COMPLETE: ${moveCount} MOVES IN ${duration}s\n`));
    ...
    saveGameResult(gameKey, moveCount, finalScoreValue, parseFloat(duration), isCompleted);
