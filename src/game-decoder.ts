...
GOAL:
Your goal is to win the game within ${MAX_STEPS} with the maximum amount of points possible.
To win the game you need to reach the 'signal_source' with 3+ decoded signals and use finalizeDiscovery() to complete the mission and unlock Omega (500 pts).

GAME RULES:
- Start at landing_zone with 30 energy
- Actions cost energy (move: 1-4 based on risk, collect:1, analyze:1, decode:2)
- planOptimalRoute() and exploreConnections() cost 0 energy, 1 step each
...
- Base: exploring areas (+10-40 based on risk), collecting artifacts (+15-100 by type), analyzing clues (+30), decoding signals (+90-150), omega revelation (+500)
- Energy bonus: 10 pts per remaining energy unit (maximize by using energy_crystal and efficient moves)
- Reflection bonus: 50 pts per hypothesis/move in planOptimalRoute (each move counts as hypothesis)

STRATEGY TIPS:
1. Plan early using planOptimalRoute() to think through your approach (0 energy, 1 step)
2. Scout adjacent areas with exploreConnections() before moving (0 energy, 1 step)
3. Prioritize collecting cost-reducing artifacts at the start
4. Balance exploration with energy conservation - lower risk paths cost less
5. Analyze signals using their NAMES only (e.g., "harmonic_sequence")
6. Combine analyzed signal names to decode patterns
7. Use parallel tool calls when possible to complete multiple actions per step

ARTIFACT ABILITIES:
- signal_scanner: Reduces decode cost from 2 to 1 energy (available at landing_zone)
- energy_crystal: Restore 5 energy with useEnergyCrystal() - one-time consumable (available at landing_zone)
- vibration_analyzer: Reduces analyze cost to 0 when used in areas with harmonic signals
- symbol_decoder: Enables translation of ancient cipher symbols
...
class DecoderGame implements Game {
    ...
    getFinalScore(): number {
        // Base score from actions (exploration, collection, analysis, decoding, omega revelation)
        // Note: Omega revelation bonus (500 pts) is added to state.score in finalizeDiscovery
        const baseScore = this.state.score;

        // Energy bonus: 10 points per remaining energy unit (can exceed 300 with energy restoration)
        const energyBonus = this.state.energy * 10;

        // Reflection bonus: 50 points per hypothesis formed during strategic planning
        const reflectionBonus = this.state.hypotheses.length * 50;

        // Return total score with all bonuses
        return baseScore + energyBonus + reflectionBonus;
    }
    ...
    init(): void {
        ...
        this.database.area = {
            ...
            signal_source: {
                ...
                artifacts: ['master_artifact'],
                signals: ['core_transmission: Omega core - integrates all patterns, solve with combined clues'],
...
class MoveToAreaTool extends BaseTool {
    ...
    public async execute(params: Record<string, unknown>): Promise<ToolResult> {
        ...
        if (!currentArea.connections.includes(area)) {
            return this.createErrorResult(`Not connected to current position (${game.state.position})`);
        }
...
class AnalyzeClueTool extends BaseTool {
    ...
    schema = z.object({
        signal: z.string().describe('Signal name to analyze (e.g., "harmonic_sequence", "basic_frequency")'),
    });
    ...
    public async execute(params: Record<string, unknown>): Promise<ToolResult> {
        ...
        const currentArea = game.database.area[game.state.position];
        const isHarmonicArea = currentArea.signals.some((s) => s.includes('harmonic') || s.includes('Gamma'));
...
class DecodeSignalTool extends BaseTool {
    ...
    public async execute(params: Record<string, unknown>): Promise<ToolResult> {
        ...
        if (game.state.decodedSignals.includes(decodedKey) || game.state.decodedSignals.includes(reverseKey)) {
...
class PlanOptimalRouteTool extends BaseTool {
    ...
    schema = z.object({
        moves: z.array(z.string()).describe(`
      An ordered array of planned moves describing your strategy. Format:
      1. Move to area A, then area B, do action C benefit X, do action D benefit Y and do action E benefit Z.
      2. ...`),
    });
    ...
    public async execute(params: Record<string, unknown>): Promise<ToolResult> {
        ...
        moves.forEach((move) => console.log(chalk.white.dim(move)));

        const game = this.game as DecoderGame;
        game.state.stepCount++;

        // Track each move as a hypothesis for reflection bonus
        game.state.hypotheses.push(...moves);

        return this.createSuccessResult({
            moves: moves,
            hypothesesTracked: moves.length,
            reflectionBonusEarned: moves.length * 50,
        });
    }
}
...
class FinalizeDiscoveryTool extends BaseTool {
    description = `
Finalize your exploration by attempting to decode the core signal.
Requirements:
- This tool can only be used with 3 or more decoded signals.
- This tool can only be used when you are at the 'signal_source' location.
`;
    ...
    public async execute(params: Record<string, unknown>): Promise<ToolResult> {
        ...
        // Calculate final score using the same logic as getFinalScore()
        // Energy bonus: 10 points per remaining energy
        const energyBonus = game.state.energy * 10;

        // Reflection bonus: 50 points per hypothesis
        const reflectionBonus = game.state.hypotheses.length * 50;

        // Calculate final score (must match getFinalScore())
        const finalScore = game.state.score + energyBonus + reflectionBonus;
