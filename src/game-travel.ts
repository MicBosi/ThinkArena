...
// User prompt
const userPrompt = `You need to visit 5 major cities worldwide with a $2000 budget. Plan your route efficiently - you have only 8 steps. Start by checking available routes, then travel strategically to visit all destinations.`;
...
class TravelGame implements Game {
  ...
  init(): void {
    ...
    this.database.routes = {
      'newyork-paris': { cost: 800, time: 1, description: 'Transatlantic flight' },
      'newyork-tokyo': { cost: 1200, time: 2, description: 'Long-haul flight' },
      'newyork-rio': { cost: 900, time: 1, description: 'South America flight' },
      'paris-tokyo': { cost: 1000, time: 2, description: 'Europe-Asia flight' },
      'paris-sydney': { cost: 1100, time: 2, description: 'Europe-Australia flight' },
      'paris-capetown': { cost: 850, time: 1, description: 'Europe-Africa flight' },
      'tokyo-sydney': { cost: 600, time: 1, description: 'Asia-Pacific flight' },
      'tokyo-capetown': { cost: 950, time: 2, description: 'Asia-Africa flight' },
      'sydney-capetown': { cost: 700, time: 1, description: 'Southern route flight' },
      'rio-capetown': { cost: 750, time: 1, description: 'Atlantic crossing flight' },
      'rio-sydney': { cost: 1050, time: 2, description: 'South America-Australia flight' },
      // Cheaper alternatives
      'paris-capetown_train': { cost: 400, time: 3, description: 'Scenic train route' },
      'newyork-rio_bus': { cost: 150, time: 4, description: 'Cross-country bus' },
      'tokyo-sydney_bus': { cost: 200, time: 5, description: 'Budget overland route' },
    };
  }
}
...
class CheckRoutesTool extends BaseTool {
  ...
  description =
    'Check all available transportation routes from your current city. Free action - no cost.';
...
class TravelTool extends BaseTool {
  ...
  description =
    'Travel to a destination city using specified route. Costs money and steps based on route.';

  schema = z.object({
    routeKey: z
      .string()
      .describe('Route key from checkRoutes (e.g., "newyork-paris", "paris-capetown_train")'),
  });
  ...
  public async execute(params: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (!routeKey.startsWith(`${game.state.currentCity}-`)) {
      return this.createErrorResult(
        `Route "${routeKey}" doesn't start from ${game.state.currentCity}`
      );
    }
    ...
    return this.createSuccessResult({
      ...
      visitBonus: game.state.visitedCities.includes(destination)
        ? 0
        : game.database.cities[destination].value,
      budgetRemaining: game.state.budget,
      stepsRemaining: MAX_STEPS - game.state.stepCount,
...
class PlanRouteTool extends BaseTool {
  ...
  public async execute(params: Record<string, unknown>): Promise<ToolResult> {
    ...
    const game = this.game as TravelGame;
    const requiredCities = ['paris', 'tokyo', 'sydney', 'rio', 'capetown'];
    const remainingCities = requiredCities.filter(
      (city) => !game.state.visitedCities.includes(city)
    );
